...
export interface AuthenticateOptions {
  ...
  /**
   * The key of the session used to set the strategy used to authenticate the user.
   */
   ...
   * If no defined, it will return the user data.
   */
  successRedirect?: string;
  /**
   * To what URL redirect in case of failed authentication.
   * If not defined, it will return the user data.
   */
  failureRedirect?: string;
  /**
   * Set if the strategy should throw and error instead of Response in case of a failed authentication.
   */
 ...
 * - `failure` is called when teh authentication failed.
 *
 * These method helps you return or throw the correct value, response or error from within
 * the strategy `authenticate` method.
 */
export abstract class Strategy<User, VerifyOptions> {
  /**
   * The name of the strategy.
   * This will be used the Authenticator to identify ans retrieve the strategy.
   */
   ...
   * to use from the Authenticator. If may receive a custom callback.
   *
   ...
   *
   * @param message The error message or a redirect to the failureRedirect.
   ...
   * @throws {Response} If a failureRedirect is set or throwOnError is false.
  ...
  protected async failure(
    ...
    // If failureRedirect is not set, we throw a 401 Response or an error
    ...
    // If we do have a failureRedirect, we redirect to it and set the error in the session errorKey.
  ...
  /**
   * Returns the user data or throe a redirect to the successRedirect.
   *
